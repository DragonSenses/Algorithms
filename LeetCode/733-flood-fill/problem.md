...
- **No Additional Structures**: Aside from the queue and a few local variables, the algorithm uses no extra data structures. The image is updated in place, so memory usage beyond the queue remains constant.
