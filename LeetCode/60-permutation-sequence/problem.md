...
function getPermutation(n: number, k: number): string {
  ...
  let result = "";
  for (let i = n; i > 0; i--) {
    const index = Math.floor(k / factorials[i - 1]); // Determine which element to pick
    result += nums[index]; // Append selected number
    nums.splice(index, 1); // Remove used element to maintain uniqueness
    k %= factorials[i - 1]; // Update k for next selection
  }
