...
### **Time Complexity**: `O(n)`
- **Bitwise Enumeration**: We iterate over 32 bit positions (constant).
- **Per-Bit Scanning**: For each bit, we scan all `n` elements to count set bits.
- **Overall Cost**: 32 * n = `O(n)` since 32 is a constant factor.
